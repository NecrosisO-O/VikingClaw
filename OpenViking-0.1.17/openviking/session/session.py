...
class Session:
    ...
    def commit(self, cause: str = "manual") -> Dict[str, Any]:
        ...
        logger.info(f"Session {self.session_id} committed (cause={cause})")
        return result

    def add_events_batch(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Add a batch of session events with idempotent event_id deduping."""
        if not events:
            return {
                "session_id": self.session_id,
                "received": 0,
        ...
        seen_ids = self._load_event_index()
        accepted = 0
        deduped = 0
        commits = 0
        ...
        for event in events:
            ...
            event_type = str(event.get("event_type", "message")).strip().lower() or "message"
            if event_type == "commit":
                cause = str(event.get("cause") or "manual")
                self.commit(cause=cause)
                commits += 1
            else:
                role_raw = str(event.get("role") or "assistant").strip().lower()
                role = role_raw if role_raw in ("user", "assistant") else "assistant"
                content = str(event.get("content") or "")
                if content:
                    self.add_message(role, [TextPart(text=content)])
    ...
    def get_context_for_search(
        ...
        # Keep legacy "summary" key for backward compatibility.
        return {
            "summaries": summaries,
            "summary": "\n\n".join(summaries),
    ...
    def _load_event_index(self) -> Set[str]:
        """Load event dedupe index from session storage."""
        ...
        index_uri = f"{self._session_uri}/.events-index.json"
        try:
            raw = run_async(self._viking_fs.read_file(index_uri))
            parsed = json.loads(raw) if raw else {}
            if isinstance(parsed, list):
                self._seen_event_ids = {str(item) for item in parsed if isinstance(item, str)}
            elif isinstance(parsed, dict):
                if isinstance(parsed.get("event_ids"), list):
                    self._seen_event_ids = {
                        str(item) for item in parsed.get("event_ids", []) if isinstance(item, str)
                    }
                else:
                    self._seen_event_ids = {
                        str(key) for key, value in parsed.items() if isinstance(key, str) and value
                    }
        except Exception:
            self._seen_event_ids = set()
    ...
    def _save_event_index(self) -> None:
        """Persist event dedupe index."""
        if not self._viking_fs or not self._events_loaded:
            return
        index_uri = f"{self._session_uri}/.events-index.json"
        payload = json.dumps({"event_ids": sorted(self._seen_event_ids)}, ensure_ascii=False)
        run_async(self._viking_fs.write_file(uri=index_uri, content=payload))
