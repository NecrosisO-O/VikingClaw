...
class SessionCompressor:
    ...
    @staticmethod
    def _abstract_from_merged_content(content: str, fallback: str = "") -> str:
        normalized = re.sub(r"\s+", " ", (content or "").strip())
        if not normalized:
            return fallback
        sentence = re.split(r"(?<=[。！？.!?])\s+", normalized, maxsplit=1)[0].strip()
        candidate = sentence or normalized
                ...
                if (
                    ...
                    try:
                        ...
                        if merged:
                            ...
                            resolved_abstract = (candidate.abstract or "").strip()
                            if not resolved_abstract:
                                resolved_abstract = self._abstract_from_merged_content(
                                    merged,
                                    fallback=target_memory.abstract,
                                )
                            target_memory.abstract = resolved_abstract
