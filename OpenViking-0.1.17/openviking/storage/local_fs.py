...
async def _enqueue_direct_vectorization(viking_fs, uri: str) -> None:
    ...
    semantic_name = uri.rstrip("/").split("/")[-1] or uri
    is_leaf = False
    abstract = ""

    try:
        stat_info = await viking_fs.stat(uri)
        is_leaf = not stat_info.get("isDir", False)
    except Exception as e:
        logger.warning(f"[local_fs] Failed to stat import root {uri}: {e}")

    try:
        if is_leaf:
            abstract = await viking_fs.read_file(uri)
        else:
            abstract = await viking_fs.abstract(uri)
    except Exception as e:
        # Imported package may not contain semantic sidecar files yet.
        logger.warning(
            f"[local_fs] Missing semantic summary for {uri}, enqueue with empty abstract: {e}"
        )
    ...
    resource = Context(
        ...
        active_count=0,
        related_uri=[],
        meta={"semantic_name": semantic_name},
    )
