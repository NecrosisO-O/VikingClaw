...
class SemanticProcessor(DequeueHandlerBase):
    ...
    async def _uri_exists(self, uri: str) -> bool:
        """Check whether a URI still exists in AGFS."""
        try:
            await get_viking_fs().stat(uri)
            return True
        except Exception:
            return False

    async def _write_semantic_file(self, uri: str, content: str) -> bool:
        """Write semantic metadata file without creating parent directories."""
        viking_fs = get_viking_fs()
        parent_uri = VikingURI(uri).parent.uri
        if not await self._uri_exists(parent_uri):
            return False

        try:
            # Bypass VikingFS.write_file() to avoid recreating removed directories.
            path = viking_fs._uri_to_path(uri)
            viking_fs.agfs.write(path, content.encode("utf-8"))
            return True
        except Exception:
            return False
    ...
    async def on_dequeue(self, data: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Resource may have been removed after enqueue; skip to avoid recreating directories.
            if not await self._uri_exists(msg.uri):
                logger.info(f"Skip semantic generation for missing URI: {msg.uri}")
                self.report_success()
                return None
    ...
    async def _process_single_directory(
        ...
        # Directory may have been removed while waiting in queue.
        if not await self._uri_exists(uri):
            logger.debug(f"Skip processing removed directory: {uri}")
            return
        ...
        # Directory may be deleted while generating summaries; don't recreate it.
        if not await self._uri_exists(uri):
            logger.debug(f"Skip writing semantic files for removed directory: {uri}")
            return

        # 5. Write files
        overview_uri = f"{uri}/.overview.md"
        abstract_uri = f"{uri}/.abstract.md"
        if not await self._write_semantic_file(overview_uri, overview):
            logger.debug(f"Skip writing semantic file for removed directory: {overview_uri}")
            return
        if not await self._write_semantic_file(abstract_uri, abstract):
            logger.debug(f"Skip writing semantic file for removed directory: {abstract_uri}")
            return
