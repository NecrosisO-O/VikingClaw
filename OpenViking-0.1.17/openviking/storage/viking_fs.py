...
class VikingFS:
    ...
    async def _tree_agent(
        ...
        async def _walk(current_path: str, current_rel: str):
            for entry in self.agfs.ls(current_path):
                ...
                rel_path = f"{current_rel}/{name}" if current_rel else name
                new_entry = {
                    "uri": self._join_uri(uri, rel_path),
    ...
    async def search(
        ...
        summary_chunks = []
        if session_info:
            raw_summaries = session_info.get("summaries")
            if isinstance(raw_summaries, list):
                summary_chunks.extend(
                    [entry.strip() for entry in raw_summaries if isinstance(entry, str) and entry.strip()]
                )
            elif isinstance(raw_summaries, str) and raw_summaries.strip():
                summary_chunks.append(raw_summaries.strip())

            legacy_summary = session_info.get("summary")
            if isinstance(legacy_summary, list):
                summary_chunks.extend(
                    [entry.strip() for entry in legacy_summary if isinstance(entry, str) and entry.strip()]
                )
            elif isinstance(legacy_summary, str) and legacy_summary.strip():
                summary_chunks.append(legacy_summary.strip())

        # Preserve order while deduplicating.
        deduped_summary_chunks = []
        for chunk in summary_chunks:
            if chunk and chunk not in deduped_summary_chunks:
                deduped_summary_chunks.append(chunk)
        session_summary = "\n\n".join(deduped_summary_chunks)
    ...
    def _join_uri(self, base_uri: str, child_path: str) -> str:
        """Join URI paths without strict scope validation."""
        part = child_path.strip("/")
        if not part:
            return base_uri
    ...
    async def _ls_original(self, uri: str, show_all_hidden: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for entry in entries:
                ...
                new_entry["uri"] = self._join_uri(uri, name)
                if entry.get("isDir"):
                    all_entries.append(new_entry)
                elif not name.startswith("."):
                    all_entries.append(new_entry)
