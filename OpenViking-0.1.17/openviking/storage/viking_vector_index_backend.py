...
class VikingVectorIndexBackend(VikingDBInterface):
    ...
    @staticmethod
    def _record_recency_key(record: Dict[str, Any]) -> Tuple[str, str]:
        """Sort key for selecting the newest record among same-URI duplicates."""
        recency = record.get("updated_at") or record.get("created_at") or ""
        return str(recency), str(record.get("id") or "")

    async def insert(self, collection: str, data: Dict[str, Any]) -> str:
        """Insert a single record."""
        coll = self._get_collection(collection)

        # Reuse existing record ID for the same URI to keep vector entries idempotent.
        record_id = data.get("id")
        uri = data.get("uri")
        if uri:
            same_uri_records = await self.filter(
            ...
            if same_uri_records:
                same_uri_records.sort(key=self._record_recency_key, reverse=True)
                canonical_id = same_uri_records[0].get("id")
                if canonical_id:
                    record_id = canonical_id

                stale_ids = []
                for item in same_uri_records:
                    item_id = item.get("id")
                    if item_id and item_id != record_id:
                        stale_ids.append(item_id)

                if stale_ids:
                    await self.delete(collection, stale_ids)
                    logger.warning(
                        f"Deduplicated {len(stale_ids)} stale record(s) for URI: {uri}"
                    )
    ...
    async def remove_by_uri(self, collection: str, uri: str) -> int:
        ...
        try:
            ...
            total_deleted = 0
            is_leaf = all(target.get("is_leaf", False) for target in target_records)
            ...
            # Delete all records for the target URI (in case duplicates exist).
            ...
            if ids_to_delete:
                deleted_count = await self.delete(collection, ids_to_delete)
