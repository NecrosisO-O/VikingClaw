...
class SessionService:
    ...
    async def commit(self, session_id: str, cause: str = "manual") -> Dict[str, Any]:
        """Commit a session (archive messages and extract memories).

        Args:
            session_id: Session ID to commit
            cause: Commit trigger cause (e.g. manual/session_end/reset/periodic)
        ...
        session.load()
        return session.commit(cause=cause)

    async def add_events_batch(self, session_id: str, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Add a batch of idempotent session events."""
        self._ensure_initialized()
        session = self.session(session_id)
        session.load()
        return session.add_events_batch(events)
