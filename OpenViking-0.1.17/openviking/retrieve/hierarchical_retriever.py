...
class HierarchicalRetriever:
    ...
    SIGNAL_TOKEN_BONUS = 0.05
    SIGNAL_TOKEN_BONUS_MAX = 0.2
    ...
    def _extract_signal_tokens(text: str) -> List[str]:
        """Extract identifier-like tokens from query text."""
        if not text:
            return []
    ...
    @classmethod
    def _signal_token_bonus(cls, query: str, abstract: str) -> float:
        """Reward exact signal-token matches to reduce cross-session leakage."""
        if not query or not abstract:
            return 0.0

        tokens = cls._extract_signal_tokens(query)
        if not tokens:
            return 0.0

        lower_abstract = abstract.lower()
        matched = sum(1 for token in tokens if token.lower() in lower_abstract)
        if matched <= 0:
            return 0.0
        return min(cls.SIGNAL_TOKEN_BONUS * matched, cls.SIGNAL_TOKEN_BONUS_MAX)
    ...
    async def _recursive_search(
        ...
        while dir_queue:
            ...
            for r, score in zip(results, query_scores):
                ...
                bonus = self._signal_token_bonus(query, r.get("abstract", ""))
                if bonus:
                    final_score += bonus
