...
class OpenVikingConfig(BaseModel):
    ...
    def from_dict(cls, config: Dict[str, Any]) -> "OpenVikingConfig":
        ...
        # Keep ov.conf backward compatibility:
        # - `server` section is used by HTTP server bootstrap only.
        # - legacy AGFS flat S3 keys are migrated to `storage.agfs.s3`.
        config_copy.pop("server", None)
        storage = config_copy.get("storage")
        if isinstance(storage, dict):
            agfs = storage.get("agfs")
            if isinstance(agfs, dict):
                legacy_s3_key_map = {
                    "s3_bucket": "bucket",
                    "s3_region": "region",
                ...
                if not isinstance(s3_cfg, dict):
                    s3_cfg = {}
                for legacy_key, s3_key in legacy_s3_key_map.items():
                    if legacy_key in agfs:
                        value = agfs.pop(legacy_key)
                        if value is not None:
                            s3_cfg.setdefault(s3_key, value)
                if s3_cfg:
                    agfs["s3"] = s3_cfg
