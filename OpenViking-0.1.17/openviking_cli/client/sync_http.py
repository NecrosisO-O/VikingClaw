...
class SyncHTTPClient:
    ...
    def commit_session(self, session_id: str, cause: Optional[str] = None) -> Dict[str, Any]:
        """Commit a session (archive and extract memories)."""
        return run_async(self._async_client.commit_session(session_id, cause=cause))

    def add_events_batch(self, session_id: str, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Add idempotent events to a session in batch."""
        return run_async(self._async_client.add_events_batch(session_id, events))
