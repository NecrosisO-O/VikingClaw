...
class AsyncHTTPClient(BaseClient):
    ...
    async def commit_session(self, session_id: str, cause: Optional[str] = None) -> Dict[str, Any]:
        """Commit a session (archive and extract memories)."""
        payload = {"cause": cause} if cause else None
        response = await self._http.post(f"/api/v1/sessions/{session_id}/commit", json=payload)
        return self._handle_response(response)
