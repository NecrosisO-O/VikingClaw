...
class TestRm:
    ...
    async def _wait_until_missing(
        self, client: AsyncOpenViking, uri: str, timeout_s: float = 2.0
    ) -> None:
        """Wait until stat(uri) fails, accounting for async background processing."""
        ...
        pytest.fail(f"URI still exists after rm: {uri}")
    ...
    async def test_rm_directory_recursive(self, client: AsyncOpenViking, sample_directory: Path):
        ...
        for data in entries:
            if data["isDir"]:
                ...
                await self._wait_until_missing(client, dir_uri)
