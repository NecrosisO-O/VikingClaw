...
class TestImportOvpack:
    ...
    async def test_import_with_default_vectorize(self, client_with_resource, temp_dir: Path):
        """Import with default vectorize=True should not fail when semantic sidecars are missing."""
        client, uri = client_with_resource

        export_path = temp_dir / "import_default_vectorize.ovpack"
        await client.export_ovpack(uri, str(export_path))

        import_uri = await client.import_ovpack(
            str(export_path), "viking://resources/import_default_vectorize/"
        )
