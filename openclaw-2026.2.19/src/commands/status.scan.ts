...
import { fetchOpenVikingObserverSnapshot } from "../memory/openviking/observer.js";
  ...
  return await withProgress(
    ...
    async (progress) => {
      ...
      const memory = await (async (): Promise<MemoryStatusSnapshot | null> => {
        ...
        const withObserver = await (async (): Promise<MemoryProviderStatus> => {
          if (status.backend !== "openviking") {
            return status;
          }
          try {
            const outboxDepth = (
              status.custom as { outbox?: { depth?: number } } | undefined
            )?.outbox?.depth;
            const observer = await fetchOpenVikingObserverSnapshot({
              cfg,
              agentId,
              outboxDepth,
              timeoutMs: Math.min(2000, opts.timeoutMs ?? 2000),
            });
            return {
              ...status,
              custom: {
            ...
            return {
              ...
              custom: {
                ...
                observer: {
                  available: false,
                  fetchedAt: Date.now(),
                  endpoint: (status.custom as { endpoint?: string } | undefined)?.endpoint ?? "",
                  timeoutMs: Math.min(2000, opts.timeoutMs ?? 2000),
                  componentsHealthy: 0,
                  componentsTotal: 0,
                  degradedComponents: [],
