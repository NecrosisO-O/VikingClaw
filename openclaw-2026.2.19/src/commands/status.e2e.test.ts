...
const mocks = vi.hoisted(() => ({
  ...
  memoryIndexGet: vi.fn(async ({ agentId }: { agentId: string }) => ({
...
describe("statusCommand", () => {
  ...
  it("shows OpenViking budget diagnostics in memory summary", async () => {
    mocks.memoryIndexGet.mockImplementationOnce(async ({ agentId }: { agentId: string }) => ({
      probeVectorAvailability: vi.fn(async () => true),
      status: () => ({
        backend: "openviking",
        provider: "openviking",
        ...
        dirty: false,
        custom: {
          outbox: { depth: 3, readyNow: 1, lastFlushDurationMs: 2100 },
          search: {
            strategy: "auto",
    ...
    runtimeLogMock.mockClear();
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0])).join("\n");
    expect(logs).toContain("inject 3180/3200");
    expect(logs).toContain("emit 4/6");
    expect(logs).toContain("dropB 2");
    expect(logs).toContain("dropN 1");
  });
