...
export async function initSessionState(params: {
  ...
  // OpenViking Context OS: ensure OpenClaw session is linked to a remote session.
  try {
    await ensureOpenVikingSessionLink({
      cfg,
      agentId,
      sessionKey,
    });
  } catch {
    // Non-fatal: session startup should not fail if Context OS is temporarily unavailable.
  }
