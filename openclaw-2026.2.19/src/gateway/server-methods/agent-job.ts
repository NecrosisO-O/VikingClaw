...
function shouldDelayErrorSnapshot(params: {
  runId: string;
  data?: Record<string, unknown>;
}): boolean {
  const hasExplicitStartTimestamp = typeof params.data?.startedAt === "number";
  const hasTrackedStart = agentRunStarts.has(params.runId);
  return hasExplicitStartTimestamp || hasTrackedStart;
}
...
function ensureAgentRunListener() {
  ...
  onAgentEvent((evt) => {
    ...
    if (phase === "error") {
      if (shouldDelayErrorSnapshot({ runId: evt.runId, data: evt.data })) {
