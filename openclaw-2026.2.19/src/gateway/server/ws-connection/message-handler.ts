      ...
      if (!client) {
        ...
        if (device && devicePublicKey && !skipPairing) {
          ...
          if (!isPaired) {
            ...
            const isScopeCovered = (scope: string): boolean => {
              ...
              // Keep pairing upgrade semantics aligned with method authorization:
              // write-capable devices can request read scope without re-pairing.
              if (scope === READ_SCOPE && allowedScopes.has(WRITE_SCOPE)) {
                return true;
              }
              return false;
            };
            if (scopes.length > 0) {
              if (pairedScopes.length === 0) {
                ...
                const missingScope = scopes.find((scope) => !isScopeCovered(scope));
                if (missingScope) {
                  logUpgradeAudit("scope-upgrade", pairedRoles, pairedScopes);
                  const ok = await requirePairing("scope-upgrade");
                  if (!ok) {
