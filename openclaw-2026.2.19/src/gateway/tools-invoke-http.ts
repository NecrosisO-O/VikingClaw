...
export async function handleToolsInvokeHttpRequest(
  ...
  if (process.env.VITEST && MEMORY_TOOL_NAMES.has(toolName)) {
    ...
    if (reasons.length > 0) {
      ...
      const slotLabel = typeof slot === "string" ? slot : "memory-core";
      sendJson(res, 400, {
        ok: false,
        error: {
          ...
          message:
            ...
            `Enable by setting plugins.slots.memory to an active slot (current: ${slotLabel}) ` +
            `for memory backend "${backend}" (and ensure plugins.enabled is not false).`,
        },
      });
      return true;
    }
