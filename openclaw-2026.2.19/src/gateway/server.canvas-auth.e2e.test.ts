...
const allowCanvasHostHttp: CanvasHostHandler["handleHttpRequest"] = async (req, res) => {
  ...
  const isA2uiPath = url.pathname === A2UI_PATH || url.pathname.startsWith(`${A2UI_PATH}/`);
  if (!isCanvasPath && !isA2uiPath) {
    return false;
  }
  res.statusCode = 200;
      ...
      run: async () => {
        await withCanvasGatewayHarness({
          ...
          run: async ({ listener, clients }) => {
            ...
            // Capability auth is validated at the gateway layer; A2UI assets may
            // be unavailable in test-only environments and return 503 instead.
