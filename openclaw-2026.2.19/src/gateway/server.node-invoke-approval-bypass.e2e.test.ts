...
describe("node.invoke approval bypass", () => {
  ...
  beforeEach(() => {
    operatorDeviceIdentity = createEphemeralDeviceIdentity();
  });
  ...
  const createEphemeralDeviceIdentity = () => {
    ...
    if (!deviceId) {
      throw new Error("failed to derive device identity");
    }
    return {
      deviceId,
      publicKeyPem,
  ...
  const buildSignedOperatorDevice = (scopes: string[]) => {
    const signedAtMs = Date.now();
    const payload = buildDeviceAuthPayload({
      deviceId: operatorDeviceIdentity.deviceId,
    ...
    return {
      id: operatorDeviceIdentity.deviceId,
      publicKey: publicKeyRawBase64UrlFromPem(operatorDeviceIdentity.publicKeyPem),
      signature: signDevicePayload(operatorDeviceIdentity.privateKeyPem, payload),
      signedAt: signedAtMs,
    };
  };
  ...
  const connectLinuxNode = async (onInvoke: (payload: unknown) => void) => {
    const nodeDeviceIdentity = createEphemeralDeviceIdentity();
    ...
    const client = new GatewayClient({
      ...
      scopes: [],
      commands: ["system.run"],
      deviceIdentity: nodeDeviceIdentity,
