...
const buildSignedDevice = (params: {
...
const connectNodeClient = async (params: {
  ...
  return await connectGatewayClient({
    ...
    deviceIdentity: createEphemeralDeviceIdentity(),
  ...
  test("enforces operator and node permissions", async () => {
    ...
    try {
      ...
      await connectOk(nodeWs, {
        token: process.env.OPENCLAW_GATEWAY_TOKEN ?? "test-gateway-token-1234567890",
        role: "node",
        client: {
        ...
        device: buildSignedDevice({
          identity: createEphemeralDeviceIdentity(),
          role: "node",
          scopes: [],
          token: process.env.OPENCLAW_GATEWAY_TOKEN ?? "test-gateway-token-1234567890",
