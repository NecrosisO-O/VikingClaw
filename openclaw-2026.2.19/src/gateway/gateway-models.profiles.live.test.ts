...
function deriveProvidersFromCandidates(params: {
  candidates: Array<Model<Api>>;
  existing: Record<string, ModelProviderConfig>;
}): Record<string, ModelProviderConfig> {
  const derived: Record<string, ModelProviderConfig> = {};

  for (const model of params.candidates) {
    if (params.existing[model.provider] || derived[model.provider]) {
      continue;
    }
    ...
    const headerEntries =
      candidate.headers && typeof candidate.headers === "object"
        ? Object.entries(candidate.headers as Record<string, unknown>).filter(
    ...
    const api = typeof candidate.api === "string" ? candidate.api : undefined;
    derived[model.provider] = {
      baseUrl,
      models: [],
      ...(api ? ({ api } as Pick<ModelProviderConfig, "api">) : {}),
      ...(headers ? { headers } : {}),
    };
  }
...
function buildLiveGatewayConfig(params: {
  ...
  // Preserve provider endpoint/headers from discovered models when
  // openclaw.json does not explicitly carry providers (profile-only setups).
  const derivedProviders = deriveProvidersFromCandidates({
    candidates: params.candidates,
    existing: baseProviders,
  });
  const nextProviders = {
    ...baseProviders,
    ...derivedProviders,
