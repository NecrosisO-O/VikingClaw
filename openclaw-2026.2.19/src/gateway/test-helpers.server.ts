...
export function installGatewayTestHooks(options?: { scope?: "test" | "suite" }) {
  ...
  if (scope === "suite") {
    beforeAll(async () => {
      ...
      await resetGatewayTestState({ uniqueConfigRoot: true, resetStateDir: true });
    });
    beforeEach(async () => {
  ...
  beforeEach(async () => {
    await setupGatewayTestHome();
    await resetGatewayTestState({ uniqueConfigRoot: false, resetStateDir: true });
  }, 60_000);
