...
const ANTHROPIC_PROXY_DEFAULT_USER_AGENT = "openclaw/bridge";
...
function isAnthropicOfficialBaseUrl(baseUrl: string): boolean {
  const trimmed = baseUrl.trim();
  if (!trimmed) {
    return false;
  }
  try {
    return new URL(trimmed).hostname.toLowerCase() === ANTHROPIC_OFFICIAL_API_HOST;
  } catch {
    return trimmed.toLowerCase().includes(ANTHROPIC_OFFICIAL_API_HOST);
  }
}
...
export function normalizeProviders(params: {
  ...
  for (const [key, provider] of Object.entries(providers)) {
    ...
    if (
      normalizedKey === "anthropic" &&
      !isAnthropicOfficialBaseUrl(normalizedProvider.baseUrl) &&
      !hasUserAgentHeader(normalizedProvider.headers)
    ) {
      mutated = true;
      normalizedProvider = {
        ...normalizedProvider,
        headers: {
          ...
          "User-Agent": ANTHROPIC_PROXY_DEFAULT_USER_AGENT,
        },
      };
    }
