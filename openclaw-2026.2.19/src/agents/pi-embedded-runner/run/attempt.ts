...
import { applyExtraParamsToAgent, stripTransportStreamErrorMarker } from "../extra-params.js";
import {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const lastAssistantFromSnapshot = messagesSnapshot
          .slice()
          .toReversed()
          ...
          | undefined;
        const transportStreamError =
          !promptError &&
          lastAssistantFromSnapshot?.stopReason === "error" &&
          typeof lastAssistantFromSnapshot.errorMessage === "string"
            ? stripTransportStreamErrorMarker(lastAssistantFromSnapshot.errorMessage)
            : undefined;
        if (transportStreamError) {
          const transportError = new Error(transportStreamError);
          transportError.name = "OpenClawPromptTransportError";
