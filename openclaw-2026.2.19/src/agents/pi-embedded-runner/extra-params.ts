...
const STREAM_TRANSPORT_ERROR_MARKER = "[openclaw:stream-transport-error]";
...
function markTransportError(message: string): string {
  return `${STREAM_TRANSPORT_ERROR_MARKER}${message}`;
}

export function stripTransportStreamErrorMarker(message?: string): string | undefined {
  if (typeof message !== "string") {
    return undefined;
  }
  return message.startsWith(STREAM_TRANSPORT_ERROR_MARKER)
    ? message.slice(STREAM_TRANSPORT_ERROR_MARKER.length)
    : undefined;
}

function createTransportErrorStream(
  model: Parameters<StreamFn>[0],
  error: unknown,
): ReturnType<StreamFn> {
  const stream = createAssistantMessageEventStream();
  const markedError = markTransportError(describeTransportError(error));
  ...
  queueMicrotask(() => {
    stream.push({
      ...
      error: {
        ...
        content: [],
        stopReason: "error",
        errorMessage: markedError,
        api: typeof modelInfo.api === "string" ? modelInfo.api : "",
        provider: typeof modelInfo.provider === "string" ? modelInfo.provider : "",
        model: typeof modelInfo.id === "string" ? modelInfo.id : "",
        usage: {
          input: 0,
          output: 0,
          cacheRead: 0,
...
function invokeStreamFnSafely(
  ...
  try {
    ...
    if (result && typeof result === "object" && "then" in result) {
      return (result as Promise<ReturnType<StreamFn>>).catch((err) =>
        createTransportErrorStream(model, err),
      ) as ReturnType<StreamFn>;
    }
    return result;
  } catch (err) {
    return createTransportErrorStream(model, err);
  }
}
...
function createStreamFnWithExtraParams(
  ...
  const wrappedStreamFn: StreamFn = (model, context, options) =>
    invokeStreamFnSafely(underlying, model, context, {
      ...streamParams,
      ...options,
    });
...
function createOpenAIResponsesStoreWrapper(baseStreamFn: StreamFn | undefined): StreamFn {
  ...
  return (model, context, options) => {
    if (!shouldForceResponsesStore(model)) {
      return invokeStreamFnSafely(underlying, model, context, options);
    }

    const originalOnPayload = options?.onPayload;
    return invokeStreamFnSafely(underlying, model, context, {
      ...options,
      onPayload: (payload) => {
