...
const isLinux = process.platform === "linux" || process.env.RUNNER_OS === "Linux";
const isWindowsCi = isCI && isWindows;
const nodeMajor = Number.parseInt(process.versions.node.split(".")[0] ?? "", 10);
// vmForks is a big win for transform/import heavy suites, but Node 24 had
// regressions with Vitest's vm runtime in this repo. Keep it explicit opt-in
// via OPENCLAW_TEST_VM_FORKS=1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.OPENCLAW_TEST_VM_FORKS === "1" && !isWindows && supportsVmForks;
...
// Reliability-first default: run groups serially unless explicitly opted into parallel groups.
const parallelGroupsEnabled = process.env.OPENCLAW_TEST_PARALLEL_GROUPS === "1";
// Keep gateway serial on Windows CI and CI by default; parallel runs can opt in
// via OPENCLAW_TEST_PARALLEL_GATEWAY=1.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.OPENCLAW_TEST_SERIAL_GATEWAY === "1" ||
  testProfile === "serial" ||
  (isCI && process.env.OPENCLAW_TEST_PARALLEL_GATEWAY !== "1");
const parallelRuns = parallelGroupsEnabled
  ? keepGatewaySerial
    ? runs.filter((entry) => entry.name !== "gateway")
    : runs
  : [];
const serialRuns = parallelGroupsEnabled
  ? keepGatewaySerial
    ? runs.filter((entry) => entry.name === "gateway")
    : []
  : runs;
  ...
  testProfile === "low"
    ...
    : testProfile === "serial"
      ...
      : testProfile === "max"
        ...
        : {
            // Reliability-first local defaults: keep worker fanout low to avoid
            // VM/heap thrash on large suites.
            unit: 2,
            unitIsolated: 1,
            extensions: 1,
            gateway: 1,
...
const needsExperimentalSqliteFlag = (() => {
  try {
    // Some Node 22 builds require this flag to expose node:sqlite.
    require("node:sqlite");
    return false;
  } catch {
    return true;
...
const DEFAULT_LOCAL_LINUX_MAX_OLD_SPACE_SIZE_MB = 6144;
const maxOldSpaceSizeMb = (() => {
  // Large local suites can hit heap limits; allow override first, then apply
  // conservative defaults for CI/local Linux.
  ...
  if (!isCI && isLinux) {
    return DEFAULT_LOCAL_LINUX_MAX_OLD_SPACE_SIZE_MB;
  }
  return null;
})();
...
function buildNodeOptions(params) {
  const flags = [...WARNING_SUPPRESSION_FLAGS];
  if (needsExperimentalSqliteFlag) {
    flags.push(EXPERIMENTAL_SQLITE_FLAG);
  }
  const withFlags = flags.reduce(
    (acc, flag) => (acc.includes(flag) ? acc : `${acc} ${flag}`.trim()),
    params.baseNodeOptions,
  );
  const heapFlag =
    params.includeHeap && maxOldSpaceSizeMb && !withFlags.includes("--max-old-space-size=")
      ? `--max-old-space-size=${maxOldSpaceSizeMb}`
      : null;
  return heapFlag ? `${withFlags} ${heapFlag}`.trim() : withFlags;
}
...
const runOnce = (entry, extraArgs = []) =>
  new Promise((resolve) => {
    ...
    const resolvedNodeOptions = buildNodeOptions({
      baseNodeOptions: process.env.NODE_OPTIONS ?? "",
      includeHeap: true,
    });
    let child;
    try {
...
if (passthroughArgs.length > 0) {
  ...
  const nextNodeOptions = buildNodeOptions({
    baseNodeOptions: process.env.NODE_OPTIONS ?? "",
    includeHeap: true,
  });
